  ...
  <div class="flex flex-col items-center p-6 bg-gray-50 min-h-screen overflow-y-auto">
    <div class="w-full max-w-4xl bg-white shadow-lg rounded-2xl p-6">
      ...
      <form @submit.prevent="currentStep === 1 ? siguientePaso() : guardarReceta" class="space-y-6">
        ...
        <!-- PASO 1: Información y Ingredientes -->
        <div v-if="currentStep === 1">
          ...
          <!-- Dificultad, Porciones, Calorías, Tiempo -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Dificultad</label>
              <select v-model="form.dificultad" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-500" required>
            ...
            <div>
              ...
              <input v-model="form.porciones" type="number" placeholder="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Calorías</label>
              <input v-model="form.calorias" type="number" placeholder="200" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Tiempo</label>
              <input v-model="form.tiempo" type="text" placeholder="30 min" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
            </div>
          </div>
          ...
          <div>
            ...
            <table class="w-full border border-gray-300 rounded-lg overflow-hidden">
              ...
              <tbody>
                <tr v-for="(item, index) in ingredientes" :key="index" class="border-b">
                  <td class="p-2"><input v-model="item.nombre" type="text" class="w-full border rounded px-2 py-1" placeholder="Ej: Harina" /></td>
                  <td class="p-2"><input v-model="item.cantidad" type="number" class="w-full border rounded px-2 py-1" placeholder="Ej: 200" /></td>
                  <td class="p-2"><input v-model="item.unidad" type="text" class="w-full border rounded px-2 py-1" placeholder="Ej: gramos" /></td>
                  <td class="p-2 text-center"><button @click="eliminarIngrediente(index)" type="button" class="text-red-500 hover:text-red-700">✕</button></td>
                </tr>
              </tbody>
            </table>
            ...
            <div class="flex justify-between mt-4">
              <button @click="agregarIngrediente" type="button" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">
                Agregar ingrediente
              </button>
...
const guardarReceta = () => {
  console.log("Datos a enviar:", { form, ingredientes, pasosPreparacion });
  alert("Receta guardada (simulación). Aquí se conectará con el backend.");
};
